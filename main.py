...
# ─── DB CONNECTION ────────────────────────────────────────────────────────────
# Set DATABASE_URL in Render → Settings → Environment Variables
# Format: postgresql://postgres:[YOUR-PASSWORD]@db.XXXX.supabase.co:5432/postgres
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://postgres:bz5nKga9MsiQe1dc@db.mykozwwwpswzukwzcvei.supabase.co:5432/postgres")
...
def get_concerts():
    with engine.connect() as conn:
        rows = conn.execute(
            text("SELECT * FROM concerts ORDER BY date")
        ).mappings().all()
